...
@implementation CYDBModel
#pragma mark - override method
+ (void)initialize
{
    if (self != [CYDBModel self]) {
        [self createTable];
    }
}
...
+ (NSDictionary *)getPropertys
{
    ...
    for (i = 0; i < outCount; i++) {
        ...
        if ([propertyType hasPrefix:@"T@"]) {
            if ([propertyType containsString:@"NSData"] || [propertyType containsString:@"NSMutableData"]) {
...
{
    __block BOOL res = NO;
    CYDBHelper *cyDB = [CYDBHelper shareInstance];
    [cyDB.dbQueue inDatabase:^(FMDatabase *db) {
...
+ (BOOL)createTable
{
    FMDatabase *db = [FMDatabase databaseWithPath:[CYDBHelper dbPath]];
    if (![db open]) {
        NSLog(@"数据库打开失败!");
        return NO;
    }
...
{
    //判断是否是CYBaseModel的子类
    for (CYDBModel *model in array) {
        if (![model isKindOfClass:[CYDBModel class]]) {
            return NO;
        }
    }
    ...
    CYDBHelper *cyDB = [CYDBHelper shareInstance];
    // 如果要支持事务
    [cyDB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        for (CYDBModel *model in array) {
